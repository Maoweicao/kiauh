...
from components.klipper_firmware.firmware_utils import run_make, run_make_clean, run_make_menuconfig
...
from utils.sys_utils import check_package_install, install_system_packages, update_system_package_lists
...
class KlipperKConfigMenu(BaseMenu):
    ...
    def __init__(self, previous_menu: Type[BaseMenu] | None=None):
        ...
        self.kconfig = self.kconfig_default if not Path(self.kconfigs_dirname).is_dir() else None
    ...
    def print_menu(self) -> None:
        cfg_found_str = Color.apply('Previously saved firmware configs found!', Color.GREEN)
...
class KlipperBuildFirmwareMenu(BaseMenu):
    ...
    def set_options(self) -> None:
        self.input_label_txt = translate('press_enter_to_install_dependencies_8e1292')
    ...
    def install_missing_deps(self, **kwargs) -> None:
        try:
            ...
            Logger.print_status(translate('installing_system_packages_5e8ef2'))
            install_system_packages(self.missing_deps)
        except Exception as e:
            Logger.print_error(e)
            Logger.print_error(translate('installing_dependencies_failed_14a646'))
        finally:
    ...
    def start_build_process(self, **kwargs) -> None:
        try:
            ...
            Logger.print_ok(translate('firmware_successfully_built_5a86f0'))
            Logger.print_ok(f"Firmware file located in '{KLIPPER_DIR}/out'!")
            if self.kconfig == self.kconfig_default:
                self.save_firmware_config()
        except Exception as e:
            Logger.print_error(e)
            Logger.print_error(translate('building_klipper_firmware_failed_9a953e'))
        finally:
            if self.previous_menu is not None:
                self.previous_menu().run()

    def save_firmware_config(self) -> None:
        Logger.print_dialog(DialogType.CUSTOM, ['You can save the firmware build configs for multiple MCUs, and use them to update the firmware after a Klipper version upgrade'], custom_title='Save firmware config')
        if not get_confirm('Do you want to save firmware config?', default_choice=False):
            return
        filename = self.kconfig_default
        while True:
            Logger.print_dialog(DialogType.CUSTOM, ["Allowed characters: a-z, 0-9 and '-'", 'The name must not contain the following:', '\n\n', '● Any special characters', "● No leading or trailing '-'"])
            input_name = get_string_input('Enter the new firmware config name', regex='^[a-z0-9]+([a-z0-9-]*[a-z0-9])?$')
        ...
        if not get_confirm(f"Save firmware config to '{filename}'?", default_choice=True):
            Logger.print_info(translate('aborted_saving_firmware_config_8b9c72'))
            return
