...
class RepoSelectMenu(BaseMenu):
    ...
    def __init__(self, name: Literal['klipper', 'moonraker'], repos: List[Repository], previous_menu: Type[BaseMenu] | None=None) -> None:
        super().__init__()
        ...
        if self.name == 'klipper':
            self.title = translate('klipper_repository_selection_menu_ca86a6')
        elif self.name == 'moonraker':
            self.title = translate('moonraker_repository_selection_menu_058cd5')

    def set_previous_menu(self, previous_menu: Type[BaseMenu] | None) -> None:
        from core.menus.settings_menu import SettingsMenu
        self.previous_menu = previous_menu if previous_menu is not None else SettingsMenu
    ...
    def set_options(self) -> None:
        ...
        for idx, repo in enumerate(self.repos, start=1):
            self.options[str(idx)] = Option(method=self.select_repository, opt_data=repo)
    ...
    def select_repository(self, **kwargs) -> None:
        ...
        Logger.print_status(f"Switching to {self.name.capitalize()}'s new source repository ...")
