...
def print_header() -> None:
    line1 = ' [ KIAUH ] '
    line2 = translate('klipper_installation_and_update_helper_e2c415')
    line3 = ''
    color = Color.CYAN
...
def print_blank_footer() -> None:
    print(translate('text_1455_cb7c99'))
...
class BaseMenu(metaclass=PostInitCaller):
    ...
    def __print_menu_title(self) -> None:
        ...
        menu_title = '╔═══════════════════════════════════════════════════════╗\n'
        if self.title:
            title = f' [ {self.title} ] ' if self.title_style == MenuTitleStyle.STYLED else self.title
            line = f'{title:~^{count}}' if self.title_style == MenuTitleStyle.STYLED else f'{title:^{count}}'
    ...
    def run(self) -> None:
        ...
        try:
            ...
            selected_option.method(opt_index=selected_option.opt_index, opt_data=selected_option.opt_data)
            self.run()
