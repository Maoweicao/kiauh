...
from components.webui_client.client_utils import get_client_status, get_current_client_config
...
class MainMenu(BaseMenu):
    ...
    def set_previous_menu(self, previous_menu: Type[BaseMenu] | None) -> None:
        translate('mainmenu_does_not_have_a_45c400')
        pass

    def set_options(self) -> None:
        self.options = {'0': Option(method=self.log_upload_menu), '1': Option(method=self.install_menu), '2': Option(method=self.update_menu), '3': Option(method=self.remove_menu), '4': Option(method=self.advanced_menu), '5': Option(method=self.backup_menu), 'e': Option(method=self.extension_menu), 's': Option(method=self.settings_menu)}

    def _init_status(self) -> None:
        status_vars = ['kl', 'mr', 'ms', 'fl', 'ks', 'cn']
        for var in status_vars:
            setattr(self, f'{var}_status', Color.apply(translate('not_installed_1aab9f'), Color.RED))
    ...
    def exit(self, **kwargs) -> None:
        Logger.print_ok(translate('happy_printing_1238ce'), False)
        sys.exit(0)
